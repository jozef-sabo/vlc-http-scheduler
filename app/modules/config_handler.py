...
class ConfigHandler:
    ...
    def create_config_folder(path: str) -> tuple:
        ...
        if ConfigHandler.config_folder_exists(path):
    ...
    def export_config(data: Union[dict, list], path: str, filename: str, mode: str = None) -> list:
        ...
        for character in mode:
            if character not in ConfigHandler.modes:
        ...
        if "x" in mode:
            ...
            config_text = xml.parseString(config_text).toprettyxml() if prettify else xml.parseString(
                config_text).toxml()
    ...
    def import_config(filepath: str, filename: str, mode: str = None) -> Union[dict, list]:
        ...
        More modes can be used simultaneously. If none provided, system tries to find mode used for export.
        If none found, exception is raised.
        ...
        if not os.path.isdir(path.path):
            raise errors.ConfigFolderMissingError(
                "Config folder ({}) for '{}' was not found.".format(filepath, filename))
        ...
        if mode:
            ...
            if not filetype:
                raise ValueError(
                    "Filetype not provided in mode {}. Valid filetypes are JSON (j) and XML (x)".format(mode))
