...
def check(connect: Connector):
    try:
        ...
        connect.status = app.modules.VLC_connector.constants.status_codes.WRONG_PORT_ERROR
        raise ConnectionRefusedError("Port is incorrect.", e)
    except TimeoutError as e:
        connect.status = app.modules.VLC_connector.constants.status_codes.CONNECTION_ERROR
    ...
    if test_request.status_code == 401:
        connect.status = app.modules.VLC_connector.constants.status_codes.AUTH_ERROR
    ...
    connect.status = app.modules.VLC_connector.constants.status_codes.OK
